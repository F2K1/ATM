...
class ATM:
    ...
    def issue_credit_card(self):
        ...
        # Not be unemployed
        # Balance >= 1000
        if self.accounts[self.current_account].job != 'unemployed' and self.accounts[self.current_account].balance >= 1000:
            ...
            print("Credit card can't be issued due to restircted privileges")
    
    def transfer(self):
        try:
            new_account = input("Insert new account no: ")
            while new_account in self.accounts: 
                print("Invalid account number")
                new_account = input("Insert new account no: ")
            else: 
                amount = int(input("Insert amount to be transfered: "))
                while amount >= int(self.accounts[self.current_account].balance or amount <= 0):
                    print("Amount must be positive and lower than (or equal to) your balance!") 
                    amount = int(input("Insert amount to be transfered: "))
                else:
                    customer = self.accounts[self.current_account]
                    customer.withdraw(amount)
                    atm.save()
                    new_account = new_account + "," + customer.age + "," + customer.job + "," + customer.marital + "," + customer.education + "," + str(amount) + "," + customer.credit_card + "\n"
